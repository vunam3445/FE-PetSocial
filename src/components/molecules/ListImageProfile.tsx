  ...
  return (
    <div id="tab-photos">
      <div className="p-6 bg-white rounded-lg shadow-sm">
        <h2 className="mb-4 text-xl font-semibold text-gray-900">Ảnh đã đăng</h2>

        <div className="grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-4">
          {media.map((item, index) => {
            if (index === media.length - 1) {
              return (
                <div ref={lastImageRef} key={item.id}>
                  <ImageProfile url={item.media_url} />
                </div>
              );
            }
            return <ImageProfile key={item.id} url={item.media_url} />;
          })}
        </div>
