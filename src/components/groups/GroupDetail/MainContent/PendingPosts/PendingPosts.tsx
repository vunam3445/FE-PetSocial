...
export const PendingPosts = ({ groupId, onActionSuccess }: PendingPostsProps) => {
  const [page, setPage] = useState(1);
  const { loading, error, setError, hasMore, posts, setPosts } =
    useGetPostsPendingOfGroup(page, groupId);
  const {
    loading: changeStatusLoading,
    error: changeStatusError,
    changeStatus,
    setError: setErrorChangePost,
  } = useChangeStatusPost();
  ...
  const lastPostRef = useCallback(
    (node: HTMLDivElement | null) => {
      ...
      if (observer.current) observer.current.disconnect(); // Ngắt kết nối cũ

      observer.current = new IntersectionObserver((entries) => {
        // Nếu phần tử đang quan sát xuất hiện trên màn hình và vẫn còn dữ liệu
        if (entries[0].isIntersecting && hasMore) {
          setPage((prev) => prev + 1); // Tăng page để gọi API trang tiếp theo
        }
      });

      if (node) observer.current.observe(node); // Bắt đầu quan sát node mới
    },
    [loading, hasMore],
  );
    ...
    <div className="max-w-4xl px-4 py-6 mx-auto sm:px-6 lg:px-8">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Các bài viết này sẽ không hiển thị trên News Feed cho đến khi bạn phê
          duyệt.
        </p>
      ...
      <div className="space-y-6">
        {posts.map((post, index) => {
          // Tính toán vị trí thứ length - 5
          const isTriggerNode =
            index === posts.length - 5 ||
            (posts.length < 5 && index === posts.length - 1);

          return (
            <div
              key={post.post_id}
              ref={isTriggerNode ? lastPostRef : undefined} //  Gắn ref vào bài viết thứ length - 5
            >
              <PreviewPost
                post={post}
      ...
      <ErrorToast
        open={changeStatusError}
        onClose={() => setErrorChangePost(false)}
        text={"Có lỗi xảy ra khi thay đổi bài viết, vui lòng thử lại sau"}
      />
    </div>
  );
};
