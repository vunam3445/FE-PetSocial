...
const NotificationItem: React.FC<ItemProps> = ({
  ...
  const [currentStatus, setCurrentStatus] = useState(data.type === 'conversation_invitation' ? data.status : null);
  const handleclick = () => {
    if (data.target_url) {
  ...
  const onInviteAction = async (e: React.MouseEvent, status: 'accepted' | 'rejected') => {
      ...
      case "conversation_invitation":
        return (
          <div className="space-y-3">
            <p className="text-sm leading-snug text-gray-700">
              <span className="font-bold text-gray-900">{data.inviter_name}</span>{" "}
              {data.message}
            </p>
            ...
            {currentStatus === 'pending' ? (
              <div className="flex gap-2">
                ...
                >
                  {isProcessing === noti.id ? <span className="w-3 h-3 border-2 rounded-full border-white/30 border-t-white animate-spin" /> : "Chấp nhận"}
                </button>
                <button
                  disabled={isProcessing === noti.id}
